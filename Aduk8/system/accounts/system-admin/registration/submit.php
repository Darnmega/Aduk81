...
}else if($data['user'] == 'subjects'){
    registerSubject($conn, $data);
}
else if($data['user'] == 'grades_levels'){
    ...
    try {
        ...
        try {
            ...
            $insert_user_info = $conn->prepare("INSERT INTO user_information 
                (user_id, first_name, middle_name, last_name, date_of_birth, nationality, id_number, marital_status) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            $insert_user_contacts = $conn->prepare("INSERT INTO user_contacts 
                (user_id, phone, email, physical, postal, place_of_residence) 
                VALUES (?, ?, ?, ?, ?, ?)");    
            $insert_user_creds = $conn->prepare("INSERT INTO user_credentials 
                (user_id, username, password, hint, role) 
                VALUES (?, ?, ?, ?, ?)");
...
function registerSubject($conn, $data)
{
    ...
    try {
        ...
        try {
            // Prepare all statements
            $insert_subject_info = $conn->prepare("INSERT INTO `subjects`(`subject_code`, `subject_name`, `classification`, `description`, _registered_by,`_on`) VALUES ( ?, ?, ?, ?, ?, NOW())");
            ...
            // Bind parameters and execute
            $insert_subject_info->bind_param("sssss", 
                $subject_code, 
                $subject_name, 
                $data['classification'],
                $data['description'],
