...
$Stafflist = getSchoolUserlist($conn, $center_no,' school_staff ');
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Aduk8 | School Staff List</title>
    <style>
        table { 
            width: 100%; 
                    ...
                    <section class="active" id="subjectSelectionSection">
                        ...
                        <div class="table-container">
                            <table id="subjectsTable">
                                ...
                                <tbody>
                                    <?php if (isset($Stafflist['message'])): ?>
                                        <tr>
                                            <td colspan="6" class="no-data"><?= htmlspecialchars($Stafflist['message']); ?></td>
                                        </tr>
                                    <?php else: ?>
                                        ...
                                        $user_results = $get_user_info->get_result();
                                        $user_info = $user_results->fetch_assoc();

                                        $get_user_acc = $conn->prepare("SELECT * FROM user_credentials WHERE user_id = ? ");
                                        $get_user_acc->bind_param("s",$user);
                                        $get_user_acc->execute();
                                        $acc_results = $get_user_acc->get_result();
                                        $user_acc_status = $acc_results->fetch_assoc();


                                        // $user_inf = getInformationUsingID($conn,$user,' user_information ',' user_id ');
                                        // $user_contacts = getInformationUsingID($conn,$user,' user_contacts ',' user_id ');
                                        // $user_acc_status = getInformationUsingID($conn,$user,' user_credentials ',' user_id ');
                                            
                                            ?>
                                            <tr onclick="window.location.href='staff-information.php?usr_id=<?= htmlspecialchars($user); ?>'">
                                                <td><?= htmlspecialchars($user_info["prefix"] . ' ' .$user_info["first_name"] . ' ' . $user_info["middle_name"] . ' ' . $user_info["last_name"]); ?></td>
                                                <td><?= htmlspecialchars($user_info["id_number"]); ?></td>
                                                <td><?= htmlspecialchars($row["occupation"]); ?></td>
                                                <td><?= htmlspecialchars($row["position"]); ?></td>
                                                <td><?= htmlspecialchars($row["status"]); ?></td>
                                                <td><?= htmlspecialchars($user_acc_status["acc_status"]); ?></td>
                                            </tr>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
                                </tbody>
                        ...
                        <div id="subjects-error" class="error-text">Please select at least one subject</div>
