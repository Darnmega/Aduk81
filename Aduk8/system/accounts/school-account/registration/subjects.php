...
$subjects = getSchoolSubjects($conn, $center_no,' NOT IN ','1');
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Aduk8 | <?= htmlspecialchars($schoolInformation['school_name'])?>'s New Subject Selection</title>
    <style>
        table { 
            width: 100%; 
...
<body>
    <div class="container">
        <main class="main-content">
            <div class="content">
                <h2><?= htmlspecialchars($schoolInformation['school_name'])?>'s New Subject Selection</h2>
                    ...
                    <section class="active" id="subjectSelectionSection">
                        ...
                        <div class="table-container">
                            <table id="subjectsTable">
                                ...
                                <tbody>
                                    <?php if (isset($subjects['message'])): ?>
                                        <tr>
                                            <td colspan="5" class="no-data"><?= htmlspecialchars($subjects['message']); ?></td>
                                        </tr>
                                    <?php else: ?>
                                        <?php foreach ($subjects as $subject): ?>
                                            <tr>
                                                <td>
                                                    <input type="checkbox" 
                                                           name="subjects[]" 
                                                           id="subject_<?= htmlspecialchars($subject["subject_code"]); ?>" 
                                                           value="<?= htmlspecialchars($subject["subject_code"]); ?>" 
                                                           class="subject-checkbox">
                                                </td>
                                                <td><?= htmlspecialchars($subject["subject_code"]); ?></td>
                                                <td><?= htmlspecialchars($subject["subject_name"]); ?></td>
                                                <td><?= htmlspecialchars($subject["classification"]); ?></td>
                        ...
                        <div class="buttons-container">        
                            ...
                            <button type="submit" id="submitBtn" class="btn-primary"><i class="fas fa-check"></i>  Register</button>
                        </div>
                    </section>
                </form>
            </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            form.addEventListener('submit', async function(e) {
                ...
                if (checkedBoxes.length === 0) {
                    ...
                    responseMessage.textContent = 'No Subject Selected. Please select at least one subject' 
                    return;
                }
